...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module DriverAutomationModule.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain DriverAutomationModule.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "DriverAutomationModule.psm1"
        } #validPSM1
